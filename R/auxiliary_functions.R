...
grow_tree_yadj=function(x, y, p, d_max, gamma){
  bf=0
  variables = sample(ncol(x), round(p*ncol(x)))#sort(sample(1:ncol(x), max(round(p*ncol(x)),2)  ))
  gammai=gamma[sample(1:length(gamma),1)]
  ...
  while(iter<=d_max){
    ...
    for(i in 1:nrow(test)){
      ...
      if(length(xtest)<=1){
        ...
        ssr=sapply(xtest,node_var_test,x=x[,test[i,"variable"]],y=y,gamma=gammai*gammascale,Pmat=Pmat,
                   terminal=test$terminal[i],middlenodes=middlenodes,betas=betas)
        ssr=t(ssr)
        ssr[is.nan(ssr)]=Inf
        best=which.min(ssr[,1])
        res=c(xtest[best],gammai*gammascale,ssr[best,])
...
node_var_test_adj=function(c0,x,y,gamma,Pmat,terminal,middlenodes,betas){
  logit=1/(1+exp(-gamma*(x-c0)))
  b0=logit*Pmat[,terminal]
  b1=(1-logit)*Pmat[,terminal]
  yres=y-as.matrix(Pmat[,-c(terminal,middlenodes)])%*%betas[-c(terminal,middlenodes)]
  yres=as.vector(yres)
  X=cbind(b0,b1)
  rank=Matrix::rankMatrix(t(X)%*%X,tol=1e-7*ncol(X))
  if(rank<ncol(X)){
  ...
  b=tryCatch(stats::coef(stats::.lm.fit(X,yres)),error=function(e)Inf)
  if(is.infinite(b[1])){
    return(c(b[1],rep(NA,ncol(X))))
  }
  e=sum((yres-X%*%b)^2)
